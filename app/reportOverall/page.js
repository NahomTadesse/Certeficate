...
const data = [
  {
    year: 2017,
    totalTransactions: '100,000 ETB',
    acceptedRequests: '2000',
    rejectedRequests: '900',
    admins: '200',
    totalDiscounts: '10,000',
  },
  {
    year: 2016,
    totalTransactions: '120,000 ETB',
    acceptedRequests: '1000',
...
export default function TableSort() {
  ...
  const downloadPDF = () => {
    const doc = new jsPDF();
    autoTable(doc, {
      head: [['Year', 'Total Transactions', 'Accepted Requests', 'Rejected Requests', 'Admins', 'Total Discounts']],
      body: sortedData.map(row => [row.year, row.totalTransactions, row.acceptedRequests, row.rejectedRequests, row.admins, row.totalDiscounts]),
    });
    ...
    <Box pos="relative">
      <ScrollArea style={{ height: '400px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Title order={2} style={{ marginTop: 45, marginLeft: 20 }}>Overall Report</Title>
        ...
        <Table horizontalSpacing="md" verticalSpacing="xs" miw={700}>
          <Table.Tbody>
            <Table.Tr>
              <Th sorted={sortBy === 'year'} reversed={reverseSortDirection} onSort={() => setSorting('year')}>
                Year
              </Th>
