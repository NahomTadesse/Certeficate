...
export default function FathersTable() {
  ...
  const [selectedFatherForTransfer, setSelectedFatherForTransfer] = useState(null);
  const [newChurch, setNewChurch] = useState(null);
  const [detailsModalOpened, { open: openDetails, close: closeDetails }] = useDisclosure(false);
  const [selectedFatherForDetails, setSelectedFatherForDetails] = useState(null);
  ...
  const handleViewDetails = (father) => {
    setSelectedFatherForDetails(father);
    openDetails();
  };
  ...
  const filteredChildren = mockChildren.filter((child) =>
    child.churchFather === selectedFatherForDetails?.name
  );

  const childrenRows = filteredChildren.map((child) => (
      ...
      <Table.Td>
        <Menu shadow="md" width={160} position="bottom-end">
          ...
          <Menu.Dropdown>
            ...
            <Menu.Item icon={<IconEye size={14} />} onClick={() => handleViewDetails(father)}>
              View Details
            </Menu.Item>
            <Menu.Divider />
            <Menu.Item icon={<IconTrash size={14} />} color="red" onClick={() => handleDelete(father.id)}>
      ...
      {loading || actionLoading ? (
        ...
        <>
          ...
          <Modal
            ...
            title={selectedFatherForDetails ? `Details for ${selectedFatherForDetails.name}` : 'Father Details'}
            size="lg"
            centered
          >
            {selectedFatherForDetails && (
              <Stack>
                <Text><strong>Name:</strong> {selectedFatherForDetails.name}</Text>
                <Text><strong>Phone:</strong> {selectedFatherForDetails.phoneNumber}</Text>
                <Text><strong>Church:</strong> {selectedFatherForDetails.churchName}</Text>
                <Text>
                  <strong>Registered:</strong>{' '}
                  {selectedFatherForDetails.registeredAt
                    ? new Date(selectedFatherForDetails.registeredAt).toLocaleDateString()
                    : 'N/A'}
                </Text>
                <Text><strong>Children:</strong> {selectedFatherForDetails.childrenCount}</Text>
                <Text><strong>Status:</strong>{' '}
                  <Text component="span" color={selectedFatherForDetails.status === 'active' ? 'green' : 'orange'} fw={500}>
                    {selectedFatherForDetails.status.charAt(0).toUpperCase() + selectedFatherForDetails.status.slice(1)}
                  </Text>
                </Text>
                <Title order={4} mt="md">Children</Title>
                <Table horizontalSpacing="md" verticalSpacing="xs" miw={600}>
                  <Table.Thead>
                    <Table.Tr>
                      <Table.Th>Name</Table.Th>
                      <Table.Th>Phone</Table.Th>
